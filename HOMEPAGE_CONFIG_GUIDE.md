...
PicKoala 支持在**管理后台**自定义首页的**特性文案**和**对比表格**，并支持多语言自动切换。

## 修改方法
1. 以管理员身份登录后台。
2. 进入 **系统设置 (Settings)** -> **首页配置 (Home Config)**。
3. 对应的 JSON 配置项会以文本框形式展现，修改后点击“保存设置”即可。

## 1. 特性文案 (Features)

控制首页顶部的特性圆点列表。
- **配置字段**: `home.features` (在后台显示为 "特性文案")
- **格式**: JSON 数组 `Array`
- **说明**: 
...
控制对比表格的表头（第一行）。
- **配置字段**: `home.table_cols` (在后台显示为 "表格列名")
- **格式**: JSON 对象 `Object`
- **说明**: 
...
控制对比表格的左侧标题列。
- **配置字段**: `home.table_rows` (在后台显示为 "表格行名")
- **格式**: JSON 对象 `Object`
- **说明**: 
    - `key` 必须严格对应：
