...
export const useSiteStore = defineStore('site', () => {
  ...
  const showAnnouncementManual = ref(0) // Used to trigger modal manually
  ...
  return {
    ...
    // Video Rate Limits
    guestVideoLimitPerMinute: () => settings.value.guest_video_limit_per_minute,
    guestVideoLimitPerHour: () => settings.value.guest_video_limit_per_hour,
    guestVideoLimitPerDay: () => settings.value.guest_video_limit_per_day,
    userVideoLimitPerMinute: () => settings.value.user_video_limit_per_minute,
    userVideoLimitPerHour: () => settings.value.user_video_limit_per_hour,
    userVideoLimitPerDay: () => settings.value.user_video_limit_per_day,
    vipVideoLimitPerMinute: () => settings.value.vip_video_limit_per_minute,
    vipVideoLimitPerHour: () => settings.value.vip_video_limit_per_hour,
    vipVideoLimitPerDay: () => settings.value.vip_video_limit_per_day,

    // Size Limit Getters (Dynamic)
    maxSizeGuestVideoMB: () => Math.round((settings.value.upload_video_max_size_guest || 0) / 1024 / 1024),
    maxSizeUserVideoMB: () => Math.round((settings.value.upload_video_max_size_user || 0) / 1024 / 1024),
    maxSizeVipVideoMB: () => Math.round((settings.value.upload_video_max_size_vip || 0) / 1024 / 1024),

    maxSizeGuestFileMB: () => Math.round((settings.value.upload_file_max_size_guest || 0) / 1024 / 1024),
    maxSizeUserFileMB: () => Math.round((settings.value.upload_file_max_size_user || 0) / 1024 / 1024),
    maxSizeVipFileMB: () => Math.round((settings.value.upload_file_max_size_vip || 0) / 1024 / 1024),
    ...
    isEpayEnabled: () => settings.value.payment_epay_enabled === true || settings.value.payment_epay_enabled === 'true',
    epayName: (locale) => {
      let name = settings.value.payment_epay_name || '易支付'
      try {
        const parsed = JSON.parse(name)
        return useSiteStore().getLocalizedText(parsed, locale)
      } catch (e) {
        return name
      }
    },
    epayLogo: () => settings.value.payment_epay_logo_url || '',
    ...
    isVipFeatureEnabled: () => {
      ...
      return (s.payment_stripe_enabled === true || s.payment_stripe_enabled === 'true') ||
        ...
        (s.payment_epay_enabled === true || s.payment_epay_enabled === 'true')
    },
    ...
    // OAuth 2.0
    isGoogleEnabled: () => settings.value.oauth_google_enabled === true || settings.value.oauth_google_enabled === 'true',
    isLinuxdoEnabled: () => settings.value.oauth_linuxdo_enabled === true || settings.value.oauth_linuxdo_enabled === 'true',
    isGithubEnabled: () => settings.value.oauth_github_enabled === true || settings.value.oauth_github_enabled === 'true',
