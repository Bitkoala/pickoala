          ...
          <div class="modal-body">
            ...
            <div class="payment-methods-grid">
               ...
               <label class="method-card" :class="{ active: paymentMethod === 'alipay' }" v-if="siteStore.isAlipayEnabled()">
                ...
                <label class="method-card" :class="{ active: paymentMethod === 'epay' }" v-if="siteStore.isEpayEnabled()">
                  <input type="radio" value="epay" v-model="paymentMethod">
                  <img v-if="siteStore.epayLogo()" :src="siteStore.epayLogo()" alt="Epay" class="method-logo epay-logo" />
                  <div v-else class="method-logo epay-logo-text">{{ $t('nav.paymentMethod.epay') }}</div>
                  <span class="method-label">{{ siteStore.epayName(locale) }}</span>
                </label>
            </div>
...
// Auto-select payment method & Access Control
watchEffect(() => {
  if (siteStore.loaded) {
    // Access Control: Redirect if VIP features are disabled
    if(!siteStore.isVipFeatureEnabled) {
      ElMessage.warning(t('vip.featureDisabled') || 'VIP feature is currently disabled')
      router.replace('/')
      return
    }
    ...
    if (!paymentMethod.value) {
      ...
      else if (siteStore.isEpayEnabled()) paymentMethod.value = 'epay'
    }
  }
})
    ...
    if (paymentMethod.value === 'stripe') {
        ...
        try {
            ...
            if (response.data.url) {
                ...
                ElMessage.error(t('vip.checkoutUrlFailed'))
            }
        } catch (error) {
            ElMessage.error(error.response?.data?.detail || t('vip.upgradeFailed'))
        } finally {
            loading.value = false
        }
        ...
        try {
            ...
            if (isMobile) {
                // Mobile: Wake up Alipay App using scheme
                // This is the standard way to trigger Alipay for "Face-to-Face" (当面付) on mobile browsers
                window.location.href = `alipays://platformapi/startapp?saId=10000007&clientVersion=3.7.0.0718&qrcode=${encodeURIComponent(qr_code)}`
                
                // Also start polling in case user comes back to this tab
                startPolling(out_trade_no)
                ElMessage.success(t('nav.useAlipayApp') || 'Opening Alipay...')
            } else {
            ...
            ElMessage.error(error.response?.data?.detail || t('vip.alipayFailed'))
        } finally {
            loading.value = false
        }
        ...
        loading.value = true
        try {
            const response = await api.post('/payments/epay/pay', {
                plan: selectedPlanObj.value.key
            })
            if (response.data.url) {
                window.location.href = response.data.url
...
const startPolling = (outTradeNo) => {
  ...
  isProcessingPayment.value = false // Reset state for new polling
  
  pollTimer = setInterval(async () => {
    // If we're already handling a success, skip
    if (isProcessingPayment.value) return
    
    try {
      const res = await api.get(`/payments/alipay/check/${outTradeNo}`)
      if (res.data.status === 'paid' && !isProcessingPayment.value) {
        // Lock immediate to prevent multiple triggers
        isProcessingPayment.value = true
        clearInterval(pollTimer)
        pollTimer = null
        
        showAlipayQrModal.value = false
        ElMessage.success(t('vip.paymentSuccess'))
        router.push('/vip/success')
      }
    } catch (e) { 
