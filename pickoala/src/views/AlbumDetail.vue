      ...
      <div class="dialog dialog--lg">
        ...
        <div class="dialog__body" v-if="selectedImage">
          ...
          <div class="detail-links">
            <div class="link-row">
              <span class="link-row__label">Direct URL</span>
              <input :value="getFullUrl(selectedImage)" readonly class="link-row__input" @focus="$event.target.select()" />
              <button class="btn btn--small" @click="copyToClipboard(getFullUrl(selectedImage))">Copy</button>
            </div>
            <div class="link-row">
...
const applyManualWatermark = async () => {
  ...
  try {
    const res = await api.post(`/images/${selectedImage.value.id}/watermark`)
    ElMessage.success(t('watermark.applySuccess'))
    onImageEdited(res.data)
  } catch (error) {
    const msg = error.response?.data?.detail || t('error.operationFailed')
    ElMessage.error(msg)
  } finally {
