  ...
  <div class="layout">
    ...
    <div class="main-wrapper">
      <main class="main-content">
        <div v-if="route.path === '/' && siteStore.isNavbarEnabled() && siteStore.navbarContent()" class="site-announcement">
          <div class="site-announcement__content">
            ...
            <div class="announcement-ticker" ref="tickerRef">
              <span 
                class="announcement-text" 
                ref="textRef"
                :class="{ 'is-scrolling': isAnnouncementOverflowing }"
              >
                {{ siteStore.navbarContent() }}
              </span>
            </div>
            <button class="announcement-detail-btn" @click="siteStore.openAnnouncement()">
    ...
    <footer class="footer">
      <div class="footer__container">
        ...
        <div class="footer__social">
          <a href="https://github.com/Bitkoala/pickoala" target="_blank" class="social-link" title="GitHub">
            <i class="fab fa-github"></i>
          </a>

          <!-- WeChat with Popover -->
          <div class="social-popover-wrapper">
            <button class="social-link wechat-btn" title="WeChat">
              <i class="fab fa-weixin"></i>
            </button>
            <div class="social-popover">
              <div class="social-popover__content">
                <img src="https://pickoala.com/img/images/2026/01/05/RM45hGH4.webp" alt="WeChat QR" />
                <p>{{ $t('common.wechatHint') }}</p>
              </div>
              <div class="social-popover__arrow"></div>
            </div>
          </div>

          <a href="https://discord.gg/QbdktDgGm8" target="_blank" class="social-link discord" title="Discord">
            <i class="fab fa-discord"></i>
          </a>
          <a href="https://t.me/BitekaolaTeam" target="_blank" class="social-link telegram" title="Telegram">
            <i class="fab fa-telegram"></i>
          </a>
          <a href="https://x.com/Bitekaola" target="_blank" class="social-link x-twitter" title="X (Twitter)">
...
// Announcement Overflow Check
const tickerRef = ref(null)
const textRef = ref(null)
const isAnnouncementOverflowing = ref(false)

const checkAnnouncementOverflow = () => {
  if (tickerRef.value && textRef.value) {
    // Add a small buffer for safety
    isAnnouncementOverflowing.value = textRef.value.offsetWidth > (tickerRef.value.offsetWidth - 20)
  }
}

watch(() => siteStore.navbarContent(), () => {
  setTimeout(checkAnnouncementOverflow, 100)
})
...
onMounted(() => {
  ...
  setTimeout(checkAnnouncementOverflow, 500) // Initial check
})
...
.social-link {
  ...
  &.wechat-btn:hover { color: #07c160; }
  &.discord:hover { color: #5865f2; }
  &.telegram:hover { color: #0088cc; }
  &.x-twitter:hover { color: var(--text-primary); }
}
...
// Site Announcement (Relocated from Navbar)
.site-announcement {
  width: 100%;
  max-width: 1400px; // Aligned with the main content card
  align-self: center;
  background: var(--bg-card);
  border: 1px solid var(--border-light);
  border-radius: var(--radius-lg);
  ...
  &__content {
    display: flex;
    align-items: center;
    gap: 16px; // Increased gap to prevent icon overlap
    width: 100%;
    overflow: hidden;
  }
}
...
.announcement-ticker {
  ...
  overflow: hidden;
  position: relative;
  // Only mask the right side to prevent cutting off text at the start
